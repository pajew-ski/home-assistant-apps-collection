...
├── addons-registry.json       # Registry of all add-ons
├── README.md
├── CLAUDE.md                  # This file
├── .github/
│   └── workflows/
...
| `addons-registry.json` | Source of truth for which add-ons are present and from where |
...
2. Enter the `slug` in the *Add-on slug to sync* field (e.g. `exocortex`).
...
- Code changes → bump the `{semver}` part in `exocortex/config.yaml`
